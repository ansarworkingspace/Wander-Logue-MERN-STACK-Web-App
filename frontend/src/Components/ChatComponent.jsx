  ...
  const fetchMessages = async () => {
    try {
      const response = await axios.get(
        `https://ansaren.online/api/users/chatMessages/${chatRoomId}`,
        {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      const { data } = await axios.post(
        `https://ansaren.online/api/users/chatSend/${chatRoomId}`,
        {
          content: message,
        },
        {
...
useEffect(() => {
  if (chatRoomId) {
    axios.get(`https://ansaren.online/api/users/participants/${chatRoomId}`, {
      withCredentials: true
    })
      .then(response => {
...
const typingHandler = (e) => {
  ...
  return (
    <div className='chatPageContainer'  style={{ maxHeight: "400px", position: "relative" }}>
      ...
      <Image style={{width:"43%",height:"80%"}} src={`https://ansaren.online/api/users/uploads/${selectedChat.profileImage}`} alt="Profile" className="followingImage" roundedCircle />
    ) : (
